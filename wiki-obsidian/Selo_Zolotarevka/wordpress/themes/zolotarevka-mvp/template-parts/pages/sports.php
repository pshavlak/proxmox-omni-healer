...
<section class="content-section">
  <div class="container">
    ...
    <!-- Футбольная команда -->
    <h2><?php echo esc_html($c['team_section_title']); ?></h2>

    <!-- Состав команды -->
    <h3>👥 <?php echo esc_html($c['team_subtitle']); ?></h3>
    <div class="cards-grid">
      <?php
      $team_q = new WP_Query([
          'post_type'      => 'sports_team',
          'posts_per_page' => (int) ($c['team_count'] ?? 20),
          'post_status'    => 'publish',
      ]);
              ...
              <tr>
                ...
                <td style="text-align:center;">
                  <span style="display:inline-flex; align-items:center; justify-content:center; gap:12px;">
                    <span style="flex:1; text-align:right; white-space:nowrap;"><strong><?php echo esc_html($home); ?></strong></span>
                    <?php if ($has_score): ?>
                    ...
                    <?php endif; ?>
                    <span style="flex:1; text-align:left; white-space:nowrap;"><strong><?php echo esc_html($away); ?></strong></span>
                  </span>
                </td>
                <td>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr<?php echo $i === 0 ? ' style="background:#d4edda;"' : ''; ?>>
            <td><?php echo $i + 1; ?></td>
            <td style="white-space:nowrap;"><strong><?php echo esc_html($row['team'] ?? ''); ?></strong></td>
    ...
    <div style="display:grid; grid-template-columns:repeat(2, 1fr); gap:12px;">
      ...
      <div class="info-card" style="border-left:4px solid #2d6a4f; padding:12px 16px;">
        <div style="font-size:0.8rem; color:#666; margin-bottom:6px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
        ...
        <div style="display:flex; align-items:center; justify-content:center; gap:8px; font-size:0.85rem;">
          <span style="flex:1; text-align:right; padding-right:2px; white-space:nowrap;"><strong><?php echo esc_html($m['home'] ?? ''); ?></strong></span>
          <span style="background:#2d6a4f; color:#fff; padding:3px 12px; border-radius:6px; font-weight:700; white-space:nowrap; flex-shrink:0;">
            <?php echo (int) ($m['score_h'] ?? 0); ?> : <?php echo (int) ($m['score_a'] ?? 0); ?>
          </span>
          <span style="flex:1; text-align:left; padding-left:2px; white-space:nowrap;"><strong><?php echo esc_html($m['away'] ?? ''); ?></strong></span>
        </div>
      </div>
      <?php endforeach; ?>
    </div>
    ...
    <!-- Фото с игр -->
    <h3><?php echo esc_html($c['gallery_title']); ?></h3>
    ...
    <!-- Другие секции -->
    <h2 id="other"><?php echo esc_html($c['other_sections_title']); ?></h2>
    <div class="cards-grid">
      <?php foreach ($c['other_sections'] as $section): ?>
        <div class="info-card">
          <div class="info-card__icon"><?php echo esc_html($section['icon'] ?? '🏐'); ?></div>
          <h3 class="info-card__title"><?php echo esc_html($section['title'] ?? ''); ?></h3>
          <p class="info-card__text"><?php echo esc_html($section['text'] ?? ''); ?></p>
        </div>
      <?php endforeach; ?>
    </div>
