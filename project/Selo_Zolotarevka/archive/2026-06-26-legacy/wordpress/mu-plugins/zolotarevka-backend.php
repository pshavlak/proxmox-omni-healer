...
final class Zolotarevka_Backend {
    ...
    public static function get_pages() {
        ...
        if ($data) return $data;
        // auto-migrate from legacy — 17 pages with hierarchy as in prototype
        $all_pages = [
            ['id' => 'home',           'name' => 'Главная',         'icon' => '🏠', 'parent' => '', 'order' => 0,  'status' => 'published'],
            ['id' => 'school',         'name' => 'Школа',           'icon' => '📚', 'parent' => '', 'order' => 1,  'status' => 'published'],
            ['id' => 'school-news',    'name' => 'Новости школы',   'icon' => '📰', 'parent' => 'school', 'order' => 0, 'status' => 'published'],
            ['id' => 'school-docs',    'name' => 'Документы',       'icon' => '📄', 'parent' => 'school', 'order' => 1, 'status' => 'published'],
            ['id' => 'kindergarten',   'name' => 'Детский сад',     'icon' => '🧸', 'parent' => '', 'order' => 2,  'status' => 'published'],
            ['id' => 'farm',           'name' => 'Совхоз',          'icon' => '🌾', 'parent' => '', 'order' => 3,  'status' => 'published'],
            ['id' => 'farm-products',  'name' => 'Продукция',       'icon' => '📦', 'parent' => 'farm', 'order' => 0, 'status' => 'draft'],
            ['id' => 'farm-vacancies', 'name' => 'Вакансии',        'icon' => '💼', 'parent' => 'farm', 'order' => 1, 'status' => 'published'],
            ['id' => 'sports',         'name' => 'Спорт',           'icon' => '⚽', 'parent' => '', 'order' => 4,  'status' => 'published'],
            ['id' => 'sports-team',    'name' => 'Команда',         'icon' => '👥', 'parent' => 'sports', 'order' => 0, 'status' => 'published'],
            ['id' => 'sports-matches', 'name' => 'Матчи',           'icon' => '🏆', 'parent' => 'sports', 'order' => 1, 'status' => 'draft'],
            ['id' => 'village-life',   'name' => 'Жизнь села',      'icon' => '🏘️', 'parent' => '', 'order' => 5,  'status' => 'published'],
            ['id' => 'village-history','name' => 'История',         'icon' => '📜', 'parent' => 'village-life', 'order' => 0, 'status' => 'published'],
            ['id' => 'village-culture','name' => 'Дом Культуры',    'icon' => '🎭', 'parent' => 'village-life', 'order' => 1, 'status' => 'draft'],
            ['id' => 'bulletin',       'name' => 'Объявления',      'icon' => '📋', 'parent' => 'village-life', 'order' => 2, 'status' => 'published'],
            ['id' => 'media',          'name' => 'Медиа',           'icon' => '📸', 'parent' => '', 'order' => 6,  'status' => 'published'],
            ['id' => 'news',           'name' => 'Новости',         'icon' => '📰', 'parent' => '', 'order' => 7,  'status' => 'published'],
        ];
        update_option('zolo_site_pages', $all_pages, false);
        return $all_pages;
    }
    ...
    public static function render() {
        ...
        <div class="zolo-wrap">
            ...
            <div class="zolo-layout">
                <!-- LEFT -->
                <div class="zolo-left">
                    <div class="zolo-panel-header" style="cursor:default;">🗂️ Золотаревка <span class="zolo-badge" id="zolo-page-count">управление</span></div>
                    <div class="zolo-nav">
        ...
        <div class="zolo-modal-overlay" id="zolo-modal-page-overlay">
            <div class="zolo-modal">
                ...
                <div class="zolo-field-row">
                    <div class="zolo-field"><label>URL-адрес (slug)</label><input type="text" id="zolo-modal-page-slug" class="zolo-input"></div>
                    <div class="zolo-field"><label>Порядок</label><input type="number" id="zolo-modal-page-order" class="zolo-input" value="99" min="0" max="999"></div>
                </div>
        ...
        <!-- Modal: edit page -->
        <div class="zolo-modal-overlay" id="zolo-edit-modal-page-overlay">
            <div class="zolo-modal">
                <h2>✏️ <span id="zolo-edit-modal-page-title">Редактирование страницы</span></h2>
                <div class="zolo-field"><label>Название страницы</label><input id="zolo-edit-page-name" class="zolo-input"></div>
                <div class="zolo-field"><label>Иконка</label>
                    <select id="zolo-edit-page-icon" class="zolo-input">
                        <option value="📄">📄 Документ</option>
                        <option value="📚">📚 Школа</option>
                        <option value="🧸">🧸 Детский сад</option>
                        <option value="🌾">🌾 Совхоз</option>
                        <option value="⚽">⚽ Спорт</option>
                        <option value="🏘️">🏘️ Жизнь села</option>
                        ...
                        <option value="🏆">🏆 Достижения</option>
                        <option value="👥">👥 Люди</option>
                    </select>
                </div>
                <div class="zolo-field"><label>Расположение</label>
                    <select id="zolo-edit-page-parent" class="zolo-input">
                        <option value="">— Корень сайта (главное меню) —</option>
                    </select>
                </div>
                <div class="zolo-field-row">
                    <div class="zolo-field"><label>URL-адрес (slug)</label><input type="text" id="zolo-edit-page-slug" class="zolo-input" placeholder="istoriya-sela"></div>
                    <div class="zolo-field"><label>Порядок</label><input type="number" id="zolo-edit-page-order" class="zolo-input" value="99" min="0" max="999"></div>
                </div>
                <div class="zolo-modal-actions">
                    <button class="zolo-btn zolo-btn-green" id="zolo-edit-modal-page-save">💾 Сохранить</button>
                    <button class="zolo-btn" onclick="zoloApp.closeModal()">Отмена</button>
                </div>
            </div>
        </div>
