...
- **Детальное логирование**: Логи всех компонентов с уровнем DEBUG
...
Система использует централизованное логирование с разделением по компонентам:

```
logs/
├── server.log    # Основной лог сервера (uvicorn)
├── app.log       # Лог приложения
├── proxmox.log   # Лог Proxmox API клиента
├── ai.log        # Лог AI агента
└── db.log        # Лог базы данных
```

Уровень логирования: **DEBUG** (настраивается в `backend/app/logger.py`)
